...
import { ProductAdminViewProps } from "../page/ProductProfile";
...
const ProductAdminView: React.FC<ProductAdminViewProps> = (inputs) => {
    ...
    const [existingImageArray, setExistingImageArray] = React.useState<
        ProductImageDto[]
    >(inputs.product?.images || []); // Array of existing image IDs
    const [addedByUserImageMap, setAddedByUserImageMap] = React.useState<
        Record<string, File>
    >({}); // Map of newly created images
    const [nextImageId, setNextImageId] = React.useState<number>(
        Math.max(...existingImageArray.map((img) => img.id ?? 0)) + 1
    ); // Id for newly created images
    const [addedByUserImageColorMap, setAddedByUserImageColorMap] =
        React.useState<Record<string, number | null>>({}); // Map of colors for newly created images
    const [addedByUserImageFileURLMap, setAddedByUserImageFileURLMap] =
        React.useState<Record<string, string>>({}); // Map of local file URLs for newly created images
    const [deleteImageArray, setDeleteImageArray] = React.useState<number[]>(
        []
    ); // Array of images to be deleted (only existing images from DB)
    ...
    const revokeImageObjectURL = (imageKey: string) => {
        const url = addedByUserImageFileURLMap
            ? addedByUserImageFileURLMap[imageKey]
            : null;
        if (url) {
    ...
    const handleDeleteImage = (imageId: string, isNew: boolean = false) => {
        ...
        const filteredImages: ProductImageDto[] | undefined =
            existingImageArray.filter((img) => img.id !== Number(imageId));
    ...
    const handleImageColorChange = (
        ...
        setExistingImageArray((prev) =>
            prev.map((img) =>
                img.id === Number(imageKey) ? { ...img, colorId } : img
            )
        );
    };
    ...
    const handleSaveChanges = async () => {
        ...
        try {
            await axios.put<ProductDto>(
                BASE_URL + ENDPOINTS.products + `/${inputs.product?.id}`,
                productForm
            );
            t("page.products.details.saveSuccess");
        } catch (error) {
            console.error("Error saving product:", error);
            t("page.products.details.saveError");
        }
    };
    ...
    const imageThumbnailControls = (
        ...
        <>
            ...
            <IconButton
                ...
                size="small"
                onClick={() =>
                    handleDeleteImage(imageId, imageId.startsWith("new-image-"))
                }
            >
                <DeleteIcon />
            </IconButton>
            ...
            <AnimatePresence>
                {!imageLoading &&
                    existingImageArray.map((img) =>
                        imageThumbnail(
                            ...
                            imageThumbnailControls(
                                String(img.imageId),
                                img.colorId as number | null
                            )
                        )
                    )}
                ...
                {addedByUserImageMap &&
                    Object.entries(addedByUserImageMap).map(([key, file]) =>
                        imageThumbnail(
                            key, // new-image-{id}
                            addedByUserImageFileURLMap
                                ? addedByUserImageFileURLMap[key]
                                : "",
                            true,
                            <Upload color="action" fontSize="small" />,
                            imageThumbnailControls(
                                key,
                                addedByUserImageColorMap[key] || null
                            )
                        )
                    )}
            </AnimatePresence>
            ...
            >
                ...
                {!imageLoading &&
                    ...
                    inputs.mainImageThumbnail(
                        selectedImageKey.startsWith("new-image-") &&
