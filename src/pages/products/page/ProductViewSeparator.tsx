...
export interface ProductAdminViewProps {
    ...
    onCancelEdit: () => void;
    onSaveSuccess: (updatedProduct?: ProductDto) => void;
}
...
const ProductViewSeparator: React.FC = () => {
    ...
    const areImagesProcessed = Object.keys(existingImageMap).length === Object.keys(product?.images || {}).length;

    // Fetch information about the product and all existing types in the database
    useEffect(() => {
        setLoading(true);
        // Create an array of API requests
        const requests: Promise<any>[] = [axios.get<ProductDto>(BASE_URL + ENDPOINTS.products + `/${productId}`)];
        ...
        Promise.all(requests)
            .then((responses) => {
                ...
                const areImagesColorBound: boolean =
                    productResponse.data?.images?.some((img) => img.colorId !== null && img.colorId !== undefined) ?? false;
    ...
    // Fetch and process product images when product is fetched
    useEffect(() => {
        if (!product) return;
            ...
            try {
                await Promise.all(
                    (product?.images ?? []).map(async (img) => {
                        if (img.imageId) {
                            const response = await fetch(BASE_URL + ENDPOINTS.product_images + `/${img.imageId}`);
                            if (response.ok) {
                                const blob = await response.blob();
                                imgMap[`existing-image-${img.imageId}`] = URL.createObjectURL(blob);
                            }
                        }
                    }),
                );
                ...
                if (product?.images?.[0] && product?.images[0]?.imageId && imgMap[`existing-image-${product?.images[0]?.imageId}`]) {
                    setSelectedImageKey(`existing-image-${product?.images[0]?.imageId}`);
                }
                setImageLoading(false);
            } catch (error) {
    ...
    // Image Thumbnails
    const imageThumbnail: ImageThumbnailType = (imageKey, imageUrl, isNewImage, prefix, controls) => (
        <Box
            key={imageKey}
            sx={{
                maxWidth: 250,
        ...
        >
            ...
            <motion.img
                ...
                style={{
                    cursor: "pointer",
                    borderRadius: 8,
                    border: imageKey === selectedImageKey ? "3px solid #b30b6dff" : "3px solid transparent",
                    width: 60,
                    height: 60,
                    objectFit: "cover",
                }}
