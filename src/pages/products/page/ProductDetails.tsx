...
const validateForm = (form: ProductDto): FormErrors => ({
    ...
    variants:
        ...
        form.variants.some(
            (v: ProductVariantDto) =>
                !v.colorId || !v.sizeId || v.quantity == null || v.quantity <= 0
        ),
});
...
const ProductDetails: React.FC = () => {
    ...
    const [imageBindings, setImageBindings] = useState<{
    ...
    const allImages = editMode
        ? [
              ...(form?.imageData?.map((img, idx) => ({
                  img,
                  key: getImageKey(img, idx),
              })) || []),
              ...(form?.imagesByColor
                  ? Object.entries(form.imagesByColor).flatMap(
                        ([colorId, arr]) =>
                            (arr || []).map((img, idx) => ({
                                img,
        ...
        ? (product.imagesByColor[selectedColorId] || []).map((img, idx) => ({
              img,
              key: `color-${selectedColorId}-${idx}`,
          }))
        : (product?.imageData || []).map((img, idx) => ({
              img,
              key: getImageKey(img, idx),
          }));
    ...
    const handleAddAttribute = () => {
        if (newAttribute.trim() && form) {
            setForm({
                ...form,
                attributes: [...(form.attributes || []), newAttribute.trim()],
            });
            setNewAttribute("");
        }
    };
    const handleDeleteAttribute = (attr: string) => {
        if (form)
            setForm({
                ...form,
                attributes: (form.attributes || []).filter((a) => a !== attr),
            });
    };
    ...
    const handleDeleteImage = (imageKey: string, isNew: boolean = false) => {
        ...
        if (isNew) {
            setImageFiles((prevFiles) =>
                prevFiles.filter(
                    (file, idx) => getImageKey(file, idx, true) !== imageKey
                )
            );
        } else {
            ...
            if (form.imageData) {
                ...
                const idx = imgs.findIndex(
                    (img, i) => getImageKey(img, i) === imageKey
                );
                if (idx !== -1) {
                    imgs.splice(idx, 1);
            ...
            if (!found && form.imagesByColor) {
                for (const colorId in form.imagesByColor) {
                    ...
                    const idx = arr.findIndex(
                        (img, i) => `color-${colorId}-${i}` === imageKey
                    );
                    if (idx !== -1) {
                        const newArr = [...arr];
                        newArr.splice(idx, 1);
    ...
    const handleChange = (field: keyof ProductDto, value: any) =>
    ...
    const handleSave = async () => {
        ...
        if (imageFiles.length > 0) {
            ...
            const newImageColorIdsArr = imageFiles.map(
                (file, idx) =>
                    imageBindings[getImageKey(file, idx, true)] ?? null
            );
            formData.append(
                "newImageColorIds",
                JSON.stringify(newImageColorIdsArr)
            );
        }
    ...
    useEffect(() => {
        if (
            ...
            if (product.colors && product.colors.length > 0) {
                firstColorId =
                    typeof product.colors[0].id === "number"
                        ? product.colors[0].id
                        : null;
            }
    ...
    const mainImageSrc = allImages[mainImageIdx]
        ? `data:image/webp;base64,${allImages[mainImageIdx].img}`
        : undefined;

    // For new uploads in edit mode
    const newImageObjs = imageFiles.map((file, idx) => ({
        file,
        key: getImageKey(file, idx, true),
    }));
    ...
    if (!product || !form)
        return (
            <Typography align="center">
                {t("page.products.details.notFound")}
            </Typography>
        );
                        ...
                        {allImages.map(({ img, key }, idx) => (
                            ...
                            >
                                <motion.img
                                    ...
                                    alt={t(
                                        "page.products.details.thumbnailAlt"
                                    )}
                        ...
                        newImageObjs.map(({ file, key }, idx) => (
                            ...
                            >
                                ...
                                <IconButton
                                    color="error"
                                    size="small"
                                    onClick={() => handleDeleteImage(key, true)}
                                >
                                    <DeleteIcon />
                                </IconButton>
                            </Box>
                        ...
                        {editMode ? (
                            <TextField
                                ...
                                helperText={
                                    formErrors.name
                                        ? t(
                                              "page.products.details.nameRequired"
                                          )
                                        : ""
                                }
                            />
                        ...
                        {editMode ? (
                            <TextField
                                ...
                                helperText={
                                    formErrors.price
                                        ? t(
                                              "page.products.details.priceRequired"
                                          )
                                        : ""
                                }
                            />
                                ...
                                <Table size="small">
                                    <TableHead>
                                        <TableRow>
                                            <TableCell>
                                                {t(
                                                    "page.products.details.color"
                                                )}
                                            </TableCell>
                                            <TableCell>
                                                {t(
                                                    "page.products.details.size"
                                                )}
                                            </TableCell>
                                            <TableCell>
                                                {t(
                                                    "page.products.details.quantity"
                                                )}
                                            </TableCell>
                                            <TableCell>
                                                {t(
                                                    "page.products.details.delete"
                                                )}
                                            </TableCell>
                                        </TableRow>
                                    </TableHead>
